...
# always one winner and one loser; there are no ties. A team receives 3 points if it wins 
# and 0 points if it loses. The winner of the tournament is the team that receives the 
...
# against all other teams exactly once. It's also guaranteed that the tournament will always 
...
def tournamentWinner_Algo(competitions, results):
    currentBestTeam = ""
...
# O(n) time | O(m) space
# where n is the number of competitions/results
# where k is the number of different teams.
# worst space case, is that every team at least won one game.
def tournamentWinner(competitions, results):
    ...
    for i in range(len(results)):
        ...
        if team in globalScore:
            globalScore[team] += 3
        else:
            globalScore[team] = 3
        
        if globalScore[team] > maxScore:
            maxScore = globalScore[team]
            champ = team
    return champ